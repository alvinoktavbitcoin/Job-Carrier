...
class Search_job extends CI_Controller
 {
	...
	public function index()
	{
		...
		$banyak		= $this->m_data->banyak_tampil_job_dari_search($kota,$kategori);
		
		$config['total_rows'] = $banyak->num_rows();
		if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
	
		$config['base_url'] = base_url().'index.php/Search_job/index';
		...
		$data['lowongan'] = $this->m_data->tampil_job_dari_search($offset,$limit,$array_id,$kota,$kategori);
